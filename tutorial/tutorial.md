...
Сейчас в ней у нас описан код, который добавляет один элемент `label` со статически заданным текстом. Давайте заменим его на кнопки, но сперва попробуем разобраться из чего же состоит данная функция.
...
Отлично. Давайте сделаем паузу и запушим все наши изменения на github. Выполнив эти действия мы ожидаем, что github за нас выполнит все сборки (как для windows, так и для Web) и обновит содержимое ветки `gh-pages`. Проверте, все ли у вас работает прежде чем двинуться дальше. 
...
Начнем с простого - с кнопки. Давайте добавим в `src/keyboard.rs` следующий код:
```rust
// ./src/keyboard.rs
use eframe::egui;  
...
```
 Вы можете заметить, как мы явно задали размер шрифта в значение 25.0, но ничто нам не мешает рассчитывать относительно длины текста который мы хотим вывести.
